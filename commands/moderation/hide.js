...
export default {
  name: "hide",
  aliases: ["اغلاق-رؤية", "اخفاء", "إخفاء"],
  category: "moderation",
  data: new SlashCommandBuilder()
    .setName("hide")
    .setDescription("إخفاء القناة الحالية عن الجميع (Hide the current channel)")
    ...
    if (commandName === "hide") {
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          .setColor("#ff3333")
          .setDescription(`👁️ **تم إخفاء القناة بنجاح** <#${channel.id}>\nبواسطة: ${interaction.user}`)
          .setTimestamp();
        await interaction.reply({ embeds: [embed] });
      } catch (err) {
        console.error(err);
        await interaction.reply({ content: "❌ فشل إخفاء القناة.", ephemeral: true });
      }
    }
  },
  async executeMessage(message, args, context) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor("#ff3333")
        .setDescription(`👁️ **تم إخفاء القناة بنجاح** <#${message.channel.id}>\nبواسطة: ${message.author}`)
        .setTimestamp();
      await message.channel.send({ embeds: [embed] });
    } catch (err) {
      console.error(err);
      return message.reply("❌ فشل إخفاء القناة.");
    }
  }
};

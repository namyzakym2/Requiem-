...
export default {
  ...
  data: new SlashCommandBuilder()
    .setName("show")
    .setDescription("إظهار القناة الحالية للجميع (Show the current channel)")
    ...
    if (commandName === "show") {
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          .setColor("#33ff33")
          .setDescription(`👁️ **تم إظهار القناة بنجاح** <#${channel.id}>\nبواسطة: ${interaction.user}`)
          .setTimestamp();
        await interaction.reply({ embeds: [embed] });
      } catch (err) {
        console.error(err);
        await interaction.reply({ content: "❌ فشل إظهار القناة.", ephemeral: true });
      }
    }
  },
  async executeMessage(message, args, context) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor("#33ff33")
        .setDescription(`👁️ **تم إظهار القناة بنجاح** <#${message.channel.id}>\nبواسطة: ${message.author}`)
        .setTimestamp();
      await message.channel.send({ embeds: [embed] });
    } catch (err) {
      console.error(err);
      return message.reply("❌ فشل إظهار القناة.");
    }
  }
};

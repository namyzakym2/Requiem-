...
export default {
  ...
  data: new SlashCommandBuilder()
    .setName("add-ron")
    .setDescription("إضافة رون لعضو (Admins / Authorized Only)")
  ...
  async executeInteraction(interaction, context) {
    ...
    try {
      ...
      if (!users[targetUser.id]) {
        users[targetUser.id] = { balance: amount, vault: 0 };
      } else {
        users[targetUser.id].balance = (users[targetUser.id].balance || 0) + amount;
      }
      save("users.json", users);
    } catch (e) {
      console.error("Error syncing users.json in add-ron:", e);
    }

    await logCurrencyTransaction(guildId, targetUser.id, amount, `Admin add by ${interaction.user.username}`, "add");

    const embed = new EmbedBuilder()
      .setColor(0x2ecc71)
      .setTitle("💵 إضافة رون")
      .setDescription(`تم إضافة **${n(amount)}** رون بنجاح إلى رصيد ${targetUser}.`)
      .setTimestamp();
  ...
  async executeMessage(message, args, context) {
    ...
    try {
      ...
      console.error("Error syncing users.json in add-ron message:", e);
    }

    await logCurrencyTransaction(guildId, targetUser.id, amount, `Admin add by ${message.author.username}`, "add");

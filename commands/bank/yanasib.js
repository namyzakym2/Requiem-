...
export default {
  ...
  async executeInteraction(interaction, context) {
    ...
    if (!users[uid]) users[uid] = { balance: userRow?.xb || 0, vault: 0 };
    users[uid].balance = Math.max(users[uid].balance || 0, userRow?.xb || 0);

    if (users[uid].balance < cost) {
      return interaction.reply({ embeds: [E("❌ رصيد غير كافٍ").setDescription(`سعر تذكرة اليانصيب هو ${n(cost)} رون.`) ], ephemeral: true });
    }
    ...
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(C).setTitle("🎟️ تم شراء تذكرة اليانصيب")
      .setDescription(`بالتوفيق! تم خصم ${n(cost)} رون من رصيدك. سيتم إعلان الفائز قريباً.`)] });
  },

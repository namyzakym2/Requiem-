...
export default {
  ...
  data: new SlashCommandBuilder()
    .setName("حماية")
    .setDescription("🛡️ اشترِ درع حماية لحماية محفظتك من اللصوص لمدة 24 ساعة (التكلفة: 5,000 رون)"),
  ...
  async executeInteraction(interaction, context) {
    ...
    if ((users[uid].balance || 0) < cost) {
      return interaction.reply({ embeds: [E("❌ رصيد غير كافٍ").setDescription(`سعر درع الحماية هو **5,000 رون**.\nرصيدك الحالي: **${n(users[uid].balance || 0)} رون**`)], ephemeral: true });
    }
    ...
    db.prepare("UPDATE leveling SET xb = xb - ? WHERE userId = ? AND guildId = ?").run(cost, uid, interaction.guildId);
  ...
  async executeMessage(message, args, context) {
    ...
    if ((users[uid].balance || 0) < cost) {
      return message.reply({ embeds: [E("❌ رصيد غير كافٍ").setDescription(`سعر درع الحماية هو **5,000 رون**.\nرصيدك الحالي: **${n(users[uid].balance || 0)} رون**`)] });
    }
    ...
    db.prepare("UPDATE leveling SET xb = xb - ? WHERE userId = ? AND guildId = ?").run(cost, uid, message.guild.id);
